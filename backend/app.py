...
def get_completed_quest_ids(user_id):
    try:
        ...
        print("Napaka pri completed_quests:", e)
        return []


def save_completed_quest(user_id, quest_id):
    supabase.table("completed_quests").upsert({
        "user_id": user_id,
        "quest_id": quest_id,
    }).execute()
...
def opravi():
    ...
    if not quest_id:
        return jsonify({"error": "Manjka quest_id"}), 400

    save_completed_quest(user_id, quest_id)
